...
export function TicketDetail({ ticket, onClose }: TicketDetailProps) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-2xl bg-white border-l border-zinc-200 flex flex-col shadow-2xl animate-in slide-in-from-right">
        
        {/* Header */}
        <div className="p-6 border-b border-zinc-200 flex items-center justify-between bg-zinc-900/50">
          <div>
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-zinc-800 text-zinc-500 transition-colors cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto flex flex-col">
          {/* Info Bar */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6 border-b border-zinc-200/80 text-sm">
            <div>
              <span className="block text-zinc-500 mb-1 text-xs">Cliente</span>
              <span className="font-medium text-zinc-800">{ticket.client}</span>
            </div>
            <div>
              <span className="block text-zinc-500 mb-1 text-xs">Abertura</span>
              <span className="text-zinc-600">{ticket.createdAt}</span>
            </div>
            <div>
            ...
            <div>
               ...
               <span className={cn(
                 ...
                 ticket.priority === 'medium' ? 'text-blue-400' : 'text-zinc-500'
               )}>
          ...
          <div className="p-6 flex-1 flex flex-col gap-8">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <div className="bg-zinc-900 border border-zinc-200 rounded-2xl rounded-tl-none p-5 text-sm text-zinc-600 leading-relaxed">
                  <div className="flex justify-between items-center mb-3 text-xs">
                    <span className="font-medium text-zinc-800">Contato do Cliente</span>
                    <span className="text-zinc-500">{ticket.createdAt}</span>
                  </div>
                  <p>{ticket.description}</p>
                  
                  {/* Attachments */}
                  <div className="mt-4 pt-4 border-t border-zinc-200/80 flex flex-wrap gap-2">
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-zinc-200 rounded-lg text-xs text-zinc-500 hover:text-accent hover:border-accent/30 cursor-pointer transition-colors">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <div className="bg-accent/5 border border-accent/10 rounded-2xl rounded-tl-none p-5 text-sm text-zinc-600 leading-relaxed">
                  <div className="flex justify-between items-center mb-3 text-xs">
                    <span className="font-medium text-zinc-800">{ticket.assignee} (Suporte)</span>
        ...
        <div className="p-6 border-t border-zinc-200 bg-white">
          <div className="bg-zinc-900 border border-zinc-200 rounded-xl overflow-hidden focus-within:border-accent/50 transition-colors">
             <textarea 
               value={reply}
               onChange={(e) => setReply(e.target.value)}
               placeholder="Digite sua resposta ou nota interna..." 
               className="w-full bg-transparent p-4 outline-none text-sm text-zinc-800 resize-none h-24 placeholder:text-zinc-500"
             ></textarea>
             <div className="bg-white p-2 border-t border-zinc-200/80 flex items-center justify-between">
               <div className="flex gap-1 items-center">
                 <button className="p-2 rounded-lg text-zinc-500 hover:text-zinc-600 hover:bg-zinc-800 transition-colors group relative">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-1 bg-transparent text-xs text-zinc-500 outline-none hover:text-zinc-600 cursor-pointer px-2 py-1 select-none font-medium"
                    >
                      ...
                      {isTypeSelectOpen && (
                        <>
                          ...
                          <motion.div
                            ...
                            className="absolute bottom-full left-2 mb-2 bg-white border border-zinc-200 rounded-xl shadow-2xl z-40 py-1 overflow-hidden min-w-[150px]"
               ...
               <button className="flex items-center gap-2 bg-accent hover:bg-accent-hover text-black font-medium px-4 py-2 rounded-lg text-sm transition-colors disabled:opacity-50" disabled={!reply.trim()}>
