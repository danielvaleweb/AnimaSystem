...
const oldStr = `  const handleUpdateStatus = async (newStatus: 'active' | 'suspended' | 'ended' | 'trial' | 'developing') => {
...
const newStr = `  const handleUpdateStatus = async (newStatus: 'active' | 'suspended' | 'ended' | 'trial' | 'developing') => {
    if (!selectedClient) return;
    if (newStatus === 'trial') {
