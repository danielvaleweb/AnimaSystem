...
content = content.replace(importRegex, "import { $1, FileText, X, ChevronDown as CD2 } from 'lucide-react';");
...
`;
content = content.replace(/  const \[clients, setClients\] = useState<ClientData\[\]>\(\[\]\);/, "  const [clients, setClients] = useState<ClientData[]>([]);" + stateInsert);
...
const buttonsTarget = `        <button 
        ...
        >`;
const buttonsReplace = `        <div className="flex gap-2 w-full sm:w-auto">
          <button 
...
`;
const returnStart = "    <div className=\"flex flex-col h-full space-y-6 relative pb-20\">";
content = content.replace(returnStart, returnStart + modalHtml);

fs.writeFileSync('src/components/clients/ClientsView.tsx', content);
console.log("Patched ClientsView.tsx");
