...
if (startIndex !== -1) {
    ...
    content = content.substring(0, startIndex) + newClientInputSection + content.substring(endIndex);
    fs.writeFileSync('src/components/finance/FinanceView.tsx', content);
    console.log("Form section replaced.");
} else {
    console.log("Target start not found");
}

// Ensure the list renders correctly
